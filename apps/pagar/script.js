...
async function processSingleItem(item) {
    try {
        const response = await fetch(`${API_URL}/pagar/contas`, {
            method: 'POST',
            headers: {
...
async function loadParcelasDoGrupo(grupoId) {
    ...
    try {
        const response = await fetch(`${API_URL}/pagar/contas/grupo/grupoId`, {
            method: 'GET',
            headers: { 
                'X-Session-Token': sessionToken,
                'Accept': 'application/json'
...
async function editarContaOtimista(editId) {
    ...
    try {
        const response = await fetch(`${API_URL}/pagar/contas/${editId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'X-Session-Token': sessionToken,
    ...
    for (let i = 0; i < atualizacoes.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(
            batch.map(async (item) => {
                const response = await fetch(`${API_URL}/pagar/contas/${item.id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-Session-Token': sessionToken,
...
window.togglePago = async function(id) {
    ...
    if (isOnline) {
        try {
            const response = await fetch(`${API_URL}/pagar/contas/${idStr}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Session-Token': sessionToken,
