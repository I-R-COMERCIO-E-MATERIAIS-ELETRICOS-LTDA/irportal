...
async function checkServerStatus() {
    try {
        const response = await fetch(`${API_URL}/receber/contas`, {
            method: 'GET',
            headers: { 
...
window.handleSubmit = async function(event) {
    ...
    try {
        const url = editId ? `${API_URL}/receber/contas/${editId}` : `${API_URL}/receber/contas`;
...
window.deleteConta = async function(id) {
    ...
    if (isOnline) {
        try {
            const response = await fetch(`${API_URL}/receber/contas/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-Session-Token': sessionToken,
                    'Accept': 'application/json'
