...
async function loadContas(showMsg = false) {
    if (!isOnline && !DEVELOPMENT_MODE) { if (showMsg) showToast('Sistema offline. Não foi possível sincronizar.', 'error'); return; }
    try {
        const response = await fetch(`${API_URL}/receber?_t=${Date.now()}`, { method:'GET', headers:{ 'X-Session-Token': sessionToken, 'Accept':'application/json', 'Cache-Control':'no-cache' }, mode:'cors' });
        if (!DEVELOPMENT_MODE && response.status === 401) { sessionStorage.removeItem('receberSession'); mostrarTelaAcessoNegado('Sua sessão expirou'); return; }
        if (!response.ok) { if (showMsg) showToast('Erro ao sincronizar dados', 'error'); return; }
        contas = await response.json();
        console.log(`✅ ${contas.length} contas carregadas`);
        updateFilters(); updateDashboard(); filterContas();
    } catch (error) { console.error('❌ Erro ao carregar contas:', error); if (showMsg) showToast('Erro ao sincronizar dados', 'error'); }
}
...
window.changeMonth = function (direction) {
    ...
    if (m > 11) { m = 0; y++; }
    if (m < 0)  { m = 11; y--; }
    currentMonth = m; currentYear = y; updateMonthDisplay();
};
...
function updateFilters() {
    ...
    if (selVend) { const cur = selVend.value; selVend.innerHTML = '<option value="">Todos Vendedores</option>'; [...vendedores].sort().forEach(v => { const o = document.createElement('option'); o.value = v; o.textContent = v; selVend.appendChild(o); }); selVend.value = cur; }
    const bancos = new Set(contas.map(c => c.banco).filter(Boolean));
    const selBanco = document.getElementById('filterBanco');
    if (selBanco) { const cur = selBanco.value; selBanco.innerHTML = '<option value="">Todos Bancos</option>'; [...bancos].sort().forEach(b => { const o = document.createElement('option'); o.value = b; o.textContent = b; selBanco.appendChild(o); }); selBanco.value = cur; }
}
window.filterContas = function () {
    ...
    if (search) filtered = filtered.filter(c => [c.numero_nf, c.orgao, c.vendedor, c.banco, c.status].some(f => f && f.toString().toLowerCase().includes(search)));
...
function renderContas(lista) {
    ...
    if (!container) return;
    if (!lista || lista.length === 0) { container.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--text-secondary);">Nenhuma conta encontrada</div>'; return; }
    const hoje = new Date().toISOString().split('T')[0];
...
function renderRow(c, hoje) {
    const isPagoTotal = c.status === 'PAGO', isParcial = /parcela/i.test(c.status||''), isPagoAlgum = isPagoTotal || isParcial, isVencido = !isPagoAlgum && c.data_vencimento && c.data_vencimento < hoje;
    ...
    let dataPgto = '-';
    if (parcelas.length > 0) { const datas = parcelas.map(p => p.data).filter(Boolean).sort(); if (datas.length > 0) dataPgto = formatDate(datas[datas.length-1]); }
    else if (c.data_pagamento) dataPgto = formatDate(c.data_pagamento);
...
function getParcelas(conta) { try { const obs = conta.observacoes; if (!obs) return []; const parsed = typeof obs === 'string' ? JSON.parse(obs) : obs; if (parsed && Array.isArray(parsed.parcelas)) return parsed.parcelas; return []; } catch { return []; } }
function getObservacoesTexto(conta) { try { const obs = conta.observacoes; if (!obs) return []; const parsed = typeof obs === 'string' ? JSON.parse(obs) : obs; if (parsed && Array.isArray(parsed.notas)) return parsed.notas; return []; } catch { return []; } }
...
window.togglePagamento = async function(id, checked) {
    ...
    if (!conta) return;
    if (checked) { const chk = document.getElementById(`chk-${id}`); if (chk) chk.checked = false; showConfirmacaoPagamentoModal(id, conta); }
    else {
        const confirm = await showConfirm(`Reverter pagamento da NF ${conta.numero_nf} para "A Receber"?`);
        if (!confirm) { const chk = document.getElementById(`chk-${id}`); if (chk) chk.checked = true; return; }
...
function showConfirm(message, options = {}) {
    return new Promise((resolve) => {
        ...
        const modalHTML = `<div class="modal-overlay" id="confirmModal" style="display:flex!important;z-index:10001!important;"><div class="modal-content confirm-modal-content" style="max-width:450px!important;"><button class="close-modal" id="confirmModalClose">✕</button><div class="confirm-modal-body"><h3 class="confirm-modal-title">${message}</h3></div><div class="modal-actions confirm-modal-actions"><button class="${type==='danger'?'danger':'success'}" id="modalConfirmBtn">${confirmText}</button><button class="secondary" id="modalCancelBtn">${cancelText}</button></div></div></div>`;
        ...
        const closeModal = (result) => { if (modal) { modal.style.animation = 'fadeOut 0.2s ease forwards'; setTimeout(() => { modal.remove(); resolve(result); }, 200); } else resolve(result); };
...
window.handleRowClick = function(event, id) { if (event.target.closest('button') || event.target.closest('input') || event.target.closest('label')) return; const c = contas.find(x => String(x.id) === String(id)); if (!c) return showToast('Conta não encontrada!', 'error'); showViewModal(c); };
window.handleEditClick = function(id) { const c = contas.find(x => String(x.id) === String(id)); if (!c) return showToast('Conta não encontrada!', 'error'); showFormModal(id); };
