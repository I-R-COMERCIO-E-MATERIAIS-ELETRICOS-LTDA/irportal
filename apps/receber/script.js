...
console.log('✅ Contas a Receber iniciado (com parcelas)');
...
function updateMonthDisplay() {
    ...
    if (el) {
        if (showAllMonths) el.textContent = `Todos — ${currentYear}`;
        else el.textContent = `${meses[currentMonth]} ${currentYear}`;
    }
    updateDashboard();
    filterContas();
}
...
function renderRow(c, hoje) {
    ...
    const valorPagoTotal = parcelas.reduce((s, p) => s + parseFloat(p.valor || 0), 0);
    let dataPgto = '-';
    const datas = parcelas.map(p => p.data_pagamento).filter(Boolean).sort();
    if (datas.length) dataPgto = formatDate(datas[datas.length - 1]);
    ...
    return `<tr class="${rowClass} row-clickable" data-id="${c.id}" onclick="handleRowClick(event, '${c.id}')">
        ...
        <td style="max-width:200px;word-wrap:break-word;">${c.orgao||'-'}</td>
        <td>${c.vendedor||'-'}</td>
        <td>${c.banco||'-'}</td>
...
function getStatusBadge(conta, hoje) {
    ...
    if (isContaEspecial(conta)) {
        const label = conta.status || conta.tipo_nf;
...
window.handleEditClick = function(id) { showFormModal(id); };
...
window.navFormTab = function(dir) {
    ...
    window.switchFormTab(tabId, btn);
};
...
function atualizarValorPagoForm() {
    ...
    if (elVP && !elVP.dataset.manualEdit) elVP.value = total > 0 ? total.toFixed(2) : '';
    const datas = _editingParcelasTemp.map(p => p.data_pagamento).filter(Boolean).sort();
    const elData = document.getElementById('f_data_pagamento');
    if (elData && !elData.dataset.manualEdit) elData.value = datas.length > 0 ? datas[datas.length-1] : '';
}
...
window.handleSubmitForm = async function(editId) {
    ...
    for (const p of parcelas) {
        if (p.valor > 0 && !p.data_pagamento) {
            showToast(`Preencha a data de pagamento da ${p.numero}ª parcela`, 'error');
            return;
        }
    }
    const valorPagoCampo = parseFloat(document.getElementById('f_valor_pago')?.value) || 0;
    const valorPago = parcelas.length > 0 ? parcelas.reduce((s,p)=>s+parseFloat(p.valor||0),0) : valorPagoCampo;
    ...
    if (parcelas.length > 0) {
        if (valorPago >= valor && valor > 0) status = 'PAGO';
        else if (valorPago > 0) status = `${parcelas.length}ª PARCELA`;
    } else if (valorPago > 0 && valor > 0 && valorPago >= valor) status = 'PAGO';

    const formData = {
        numero_nf, orgao, vendedor, banco, tipo_nf, valor, data_emissao, data_vencimento, data_pagamento, status,
        parcelas: parcelas.map(p => ({ numero: p.numero, valor: p.valor, data_pagamento: p.data_pagamento, data_vencimento: data_vencimento }))
    };
...
// ============================================
// TOGGLE PAGAMENTO E CONFIRMAÇÕES
// ============================================
...
function showConfirm(message, options = {}) {
    return new Promise((resolve) => {
        ...
        const modalHTML = `<div class="modal-overlay" id="confirmModal" style="display: flex !important;"><div class="modal-content confirm-modal-content"><button class="close-modal" onclick="document.getElementById('confirmModal').remove()">✕</button><div class="confirm-modal-body"><h3 class="confirm-modal-title">${message}</h3></div><div class="confirm-modal-actions"><button class="success" id="modalConfirmBtn">${confirmText}</button><button class="danger" id="modalCancelBtn">${cancelText}</button></div></div></div>`;
...
function showDeleteConfirmation(message, onConfirm) {
    const modalHTML = `<div class="modal-overlay" id="customDeleteModal" style="display: flex !important;"><div class="modal-content confirm-modal-content"><button class="close-modal" onclick="document.getElementById('customDeleteModal').remove()">✕</button><div class="confirm-modal-body"><h3 class="confirm-modal-title">${message}</h3></div><div class="confirm-modal-actions"><button class="success" id="customDeleteConfirmBtn">Sim</button><button class="danger" id="customDeleteCancelBtn">Cancelar</button></div></div></div>`;
    document.body.insertAdjacentHTML('beforeend', modalHTML);
...
function showConfirmacaoPagamentoModal(id, conta) {
    const modalHTML = `<div class="modal-overlay" id="confirmPagModal" style="display: flex !important;"><div class="modal-content confirm-modal-content"><button class="close-modal" onclick="document.getElementById('confirmPagModal').remove()">✕</button><div class="confirm-modal-body"><h3 class="confirm-modal-title">O pagamento da NF ${conta.numero_nf} será parcelado?</h3></div><div class="confirm-modal-actions"><button class="success" id="btnSim">Sim</button><button class="danger" id="btnNao">Não</button><button class="secondary" id="btnCancelar">Cancelar</button></div></div></div>`;
    document.body.insertAdjacentHTML('beforeend', modalHTML);
    ...
    btnSim.onclick = () => { close(); showFormModal(id, true); };
    btnNao.onclick = () => { close(); showFormModalPagamentoIntegral(id, conta); };
...
console.log('✅ Script contas a receber carregado com sucesso (nova estrutura de parcelas)');
