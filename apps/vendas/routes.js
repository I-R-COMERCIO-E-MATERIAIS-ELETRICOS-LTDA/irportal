...
module.exports = function (supabase) {
    ...
    // Chave composta: numero_nf + vendedor (ambos normalizados)
    ...
    function sanitize(record) {
        const { id, id_controle_frete, id_contas_receber, observacoes, numero_parcela, chave_parcela, prioridade, created_at, ...safe } = record;
        return safe;
    }
    ...
    router.post('/sincronizar', async (req, res) => {
        try {
            ...
            // Índice de contas por chave composta (NF + vendedor)
            const contasPorChave = {};
            for (const conta of contas) {
                const vendedorConta = (conta.vendedor || '').toUpperCase().trim();
                const key = makeKey(conta.numero_nf, vendedorConta);
                const atual = contasPorChave[key];
                if (!atual || conta.status === 'PAGO' || (atual.status !== 'PAGO' && conta.data_pagamento && !atual.data_pagamento)) {
                    contasPorChave[key] = conta;
                }
            }
            ...
            for (const frete of fretes) {
                const vendedorFrete = (frete.vendedor || '').toUpperCase().trim();
                const key = makeKey(frete.numero_nf, vendedorFrete);
                mapa[key] = {
                    numero_nf: normalizeNF(frete.numero_nf),
                    vendedor: vendedorFrete,
            ...
            // Mescla dados de pagamento
            let matched = 0, unmatchedContas = 0;
            for (const [key, conta] of Object.entries(contasPorChave)) {
                const vendedorConta = (conta.vendedor || '').toUpperCase().trim();
            ...
            if (!registros.length) return res.json({ success: true, message: 'Nenhum registro para sincronizar', total: 0 });
