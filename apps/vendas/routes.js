    ...
    router.post('/sincronizar', async (req, res) => {
        try {
            ...
            for (const frete of fretes) {
                const key = `${frete.numero_nf}||${frete.vendedor}`;
                mapa[key] = {
                    ...
                    nome_orgao: frete.nome_orgao || frete.orgao,
                    ...
                    status_frete: (frete.status || 'EM TRÂNSITO').replace('_', ' '),
            ...
            for (const conta of contas) {
                ...
                if (mapa[key]) {
                    ...
                    mapa[key].origem = statusPagamento === 'PAGO' ? 'PAGO (Frete+Conta)' : 'PARCIAL (Frete+Conta)';
                } else {
                    mapa[key] = {
            ...
            if (!registros.length) return res.json({ success: true, message: 'Nenhum registro' });
            const CHUNK = 200;
            let erros = 0;
            for (let i = 0; i < registros.length; i += CHUNK) {
                const chunk = registros.slice(i, i+CHUNK);
                const { error: upsertError } = await supabase.from('vendas').upsert(chunk, { onConflict: 'numero_nf, vendedor' });
                if (upsertError) { console.error(`Erro lote ${i}:`, upsertError.message); erros++; }
            }
