...
async function carregarTudo() {
    try {
        const [marcasRes, precosRes] = await Promise.all([
            fetchWithTimeout(`${API_URL}/precos/marcas`, { method: 'GET', headers: getHeaders() }),
...
async function atualizarMarcas() {
    try {
        const response = await fetchWithTimeout(`${API_URL}/precos/marcas`, {
            method: 'GET',
            headers: getHeaders()
        });
        if (response.ok) {
...
function filterPrecos() {
    ...
    searchDebounceTimer = setTimeout(() => { loadPrecos(1); }, 200);
}
